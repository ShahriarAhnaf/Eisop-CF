...
\subsectionAndLabel{What do redundant null check warnings mean?}{faq-redundant-null-check-warnings}

\item
  \textbf{Redundant null check warnings} (e.g., \texttt{[nulltest.redundant]}) indicate that you are performing a null check on a value that the type system has already determined cannot be null. For example:
  \begin{Verbatim}
...
\begin{itemize}
\item
  For redundant null check warnings, you can safely remove the null check since the type system has already verified the value cannot be null. Find out more on what the nullness checker guarantees in section \ref{nullness-checks}.
\end{itemize}
