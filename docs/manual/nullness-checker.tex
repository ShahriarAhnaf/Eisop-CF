...
See Section~\ref{nullness-checker-guarantee} for more details about
...
\sectionAndLabel{What the Nullness Checker guarantees}{nullness-checker-guarantee}
...
\begin{Verbatim}
  ...
  if (nnobj == null)     // checker warning:  redundant test against null
\end{Verbatim}
...
guarantee (see Section~\ref{nullness-checker-guarantee}) that your program will not
...
\subsubsectionAndLabel{``static field instrumentation \ldots not initialized''}{static-initialization-warnings}
\begin{verbatim}
warning: [initialization.static.field.uninitialized] static field myStaticField not initialized
        private static volatile myClass myStaticField;
                                ^
        1 warning
\end{verbatim}
This warning is issued when a static field is not initialized.  The Initialization Checker requires that all static fields be initialized for the Nullness Checker to guarantee that the null pointer exception does not happen.
Find out more on what the Nullness Checker guarantees in section \ref{nullness-checker-guarantee}.
